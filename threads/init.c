...
int main(void) {
  ...
  /* pintos -v -- -q run alarm-multiple 이라고 한다면,
  run과 alarm-multiple이 실행인자가 된다*/
  argv = read_command_line(); // argv = ["-q", "run","alarm-multiple"]
  argv = parse_options(argv); // argv = ["run","alarm-multiple"]
